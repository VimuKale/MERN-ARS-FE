...
import { RegisterAppAdmin, UpdateAppAdmin } from "../../../slices/uruSlice";
...
const AdminAddUpdate = () => {
  ...
  const [firstName, setFirstName] = useState(
    stateData ? stateData.a_firstName : ""
  );
  const [lastName, setLastName] = useState(
    stateData ? stateData.a_lastName : ""
  );
  const [phone, setPhone] = useState(stateData ? stateData.a_phone : "");
  const [email, setEmail] = useState(stateData ? stateData.email : "");
  const [password, setPassword] = useState(stateData ? stateData.password : "");
  const [address, setAddress] = useState(stateData ? stateData.a_address : "");
  const [city, setCity] = useState(stateData ? stateData.a_city : "Pune");
  const [state, setState] = useState(
    stateData ? stateData.a_state : "Maharashtra"
  );
  const [zip, setZip] = useState(stateData ? stateData.a_zip : "");
  ...
  const handleOnSubmit = async (e) => {
    ...
    if (validate()) {
      ...
      setLoading(true);
      dispatch(RegisterAppAdmin(userDetails))
        .unwrap()
        .then((res) => {
          setLoading(false);
  ...
  const handleOnUpdate = async (e) => {
    ...
    if (validate()) {
      ...
      setLoading(true);
      dispatch(UpdateAppAdmin(userDetails))
        .unwrap()
        .then((res) => {
          setLoading(false);
          clearform();
    ...
    >
      ...
      >
        <Button
          ...
          onClick={() => {
            stateData ? navigate("/admindashboard") : navigate("/");
          }}
        >
        ...
        <Grid item xs={12} sm={12} md={12} lg={12}>
          <Paper elevation={24} sx={{ padding: "2rem", borderRadius: "25px" }}>
            ...
            >
              {stateData ? "Update Admin" : "Register Admin"}
            </Typography>
